    ...
    const TSet<FName>& AllParam)
{
    ...
    for (const FName& It : AllParam)
    {
        ...
        if (const FProperty* Property = InClass->FindPropertyByName(It))
        {
            ...
            {
                const bool IsExposedToSpawn = Property->HasMetaData(TEXT("ExposeOnSpawn")) || Property->HasAllPropertyFlags(CPF_ExposeOnSpawn);
    ...
    for (const FName& It : AllFunctions)
    {
        ...
        if (const UFunction* Function = InClass->FindFunctionByName(It))
        {
            ...
            if (IsValidFunction)
            {
                if (IsValidFunction && (Function->GetBoolMetaData(FName("ExposeAutoCall")) || ItStr.StartsWith(InitPrefix)))
                {
    ...
    Algo::Sort(
        ...
        [](const FName& A, const FName& B)
        {
            ...
            const bool IsAInit = AStr.StartsWith(InitPrefix);
            const bool IsBInit = BStr.StartsWith(InitPrefix);
            return (IsAInit && NOT IsBInit) ||
                ((IsAInit && IsBInit) ? LessSuffix(A, AStr, B, BStr) : A.FastLess(B));
        });

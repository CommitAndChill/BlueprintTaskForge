...
void UBtf_ExtendConstructObject_K2Node::PostEditChangeProperty(FPropertyChangedEvent& e)
{
    ...
    auto IsDirty = false;
    const auto& PropertyName = (e.Property != NULL) ? e.Property->GetFName() : NAME_None;
...
void UBtf_ExtendConstructObject_K2Node::Serialize(FArchive& Ar)
{
    ...
    if (Ar.IsSaving())
    {
        if (ProxyClass && NOT ProxyClass->IsChildOf(UObject::StaticClass()))
        {
            ProxyClass = nullptr;
        }
    }
...
void UBtf_ExtendConstructObject_K2Node::ValidateNodeDuringCompilation(class FCompilerResultsLog& MessageLog) const
{
    ...
    if (NOT (ProxyClass && ClassPin && ClassPin->Direction == EGPD_Input))
    {
        MessageLog.Error(
            *FText::Format(
...
void UBtf_ExtendConstructObject_K2Node::GeneratePinTooltip(UEdGraphPin& Pin) const
{
    ...
    check(Schema);
    const auto& K2Schema = Cast<const UEdGraphSchema_K2>(Schema);
...
bool UBtf_ExtendConstructObject_K2Node::HasExternalDependencies(TArray<class UStruct*>* OptionalOutput) const
{
    ...
    const auto& ProxyFactoryResult = (ProxyFactoryClass != nullptr) && (ProxyFactoryClass->ClassGeneratedBy != SourceBlueprint);
    if (ProxyFactoryResult && OptionalOutput)
    {
        OptionalOutput->AddUnique(ProxyFactoryClass);
    }

    const auto& ProxyResult = (ProxyClass != nullptr) && (ProxyClass->ClassGeneratedBy != SourceBlueprint);
    if (ProxyResult && OptionalOutput)
    {
...
bool UBtf_ExtendConstructObject_K2Node::UseWorldContext() const
{
    ...
    const auto& ParentClass = BP ? BP->ParentClass : nullptr;
...
void UBtf_ExtendConstructObject_K2Node::PinDefaultValueChanged(UEdGraphPin* Pin)
{
    if (Pin->PinName == ClassPinName)
    {
        ...
        {
            const auto& N = Pins[PinIndex]->PinName;
...
void UBtf_ExtendConstructObject_K2Node::RefreshNames(TArray<FBtf_NameSelect>& NameArray, const bool RemoveNone) const
{
    ...
    auto UniquePred = [&Arr = NameArray](const FBtf_NameSelect& A, const FBtf_NameSelect& B)
    {
        ...
        {
            const auto& ID = Arr.FindLast(B.Name);
...
void UBtf_ExtendConstructObject_K2Node::ResetPinByNames(TSet<FName>& NameArray)
{
    ...
    {
        auto* OldPin = FindPin(OldPinReference);
        if (OldPin)
        {
            Pins.Remove(OldPin);
...
void UBtf_ExtendConstructObject_K2Node::AddAutoCallFunction(const FName PinName)
{
    ...
    {
        if (NOT ExecFunction.Remove(PinName))
        {
...
void UBtf_ExtendConstructObject_K2Node::AddInputExec(const FName PinName)
{
    ...
    {
        if (NOT AutoCallFunction.Remove(PinName))
        {
            if (auto* Pin = FindPin(PinName))
            {
                RemovePin(Pin);
...
void UBtf_ExtendConstructObject_K2Node::RemoveNodePin(const FName PinName)
{
    ...
    if (NOT ExecFunction.Remove(PinName))
    {
        if (NOT AutoCallFunction.Remove(PinName))
        {
            if (NOT OutDelegate.Remove(PinName))
            {
                if (NOT InDelegate.Remove(PinName))
                {
...
void UBtf_ExtendConstructObject_K2Node::CreatePinsForClass(UClass* InClass, const bool FullRefresh)
{
...

    {
        ...
        const auto& ValidPropertyPred = [InCl = InClass](const FName& Name)
        {
            if (Name != NAME_None)
            {
                const auto& Property = InCl->FindPropertyByName(Name);
                if (Property == nullptr)
                {
                    return true;
                }
        ...
        const auto& ValidFunctionPred = [InCl = InClass](const FName& Name)
        {
            if (Name != NAME_None)
            {
                const auto& Property = InCl->FindFunctionByName(Name);
                if (Property == nullptr)
                {
                    return true;
                }
        ...
        if (AllParam.Num())
        {
            ...
            const auto& Count = Arr.RemoveAll(ValidPropertyPred);
            if (Count)
            {
        ...
        {
            {
                ...
                const auto& Count = Arr.RemoveAll(ValidFunctionPred);
                if (Count)
                {
        ...
        if (AllFunctionsExec.Num())
        {
...

            {
                ...
                const auto& Count = Arr.RemoveAll(PureFunctionPred);
                if (Count)
                {
...
void UBtf_ExtendConstructObject_K2Node::CollectSpawnParam(UClass* InClass, const bool FullRefresh)
{
    ...
    for (TFieldIterator<UField> It(InClass); It; ++It)
    {
        if (const auto& LocFunction = Cast<UFunction>(*It))
        {
                ...
                NOT FObjectEditorUtils::IsFunctionHiddenFromClass(LocFunction, InClass))
            {
                ...
                if (NewFunction)
                {
                    if (FunctionName.ToString().StartsWith(FNames_Helper::InPrefix) && NOT LocFunction->HasAnyFunctionFlags(FUNC_BlueprintPure | FUNC_Const))
                    {
                        if (FullRefresh)
                        {
    ...
    for (TFieldIterator<FProperty> PropertyIt(InClass); PropertyIt; ++PropertyIt)
    {
        if (const auto& DelegateProperty = CastField<FMulticastDelegateProperty>(*PropertyIt))
        {
            ...
            {
                if (NOT LocFunction->HasAllFunctionFlags(/*FUNC_BlueprintCallable |*/ FUNC_Public) ||
            ...
            else
            {
                if (DelegateName.ToString().StartsWith(FNames_Helper::InPrefix) ||
                    (DelegateName.ToString().StartsWith(FNames_Helper::OutPrefix) && NOT AllowOutDelegate))
                {
                    InDelegate.AddUnique(DelegateName);
                }
                else if (DelegateName.ToString().StartsWith(FNames_Helper::OutPrefix) && AllowOutDelegate)
                {
    ...
    Algo::Sort(
        ...
        [](const FName& A, const FName& B)
        {
            ...
            const auto& AInit = AStr.StartsWith(FNames_Helper::InitPrefix);
            const auto& BInit = BStr.StartsWith(FNames_Helper::InitPrefix);
